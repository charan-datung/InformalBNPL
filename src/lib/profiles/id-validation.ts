...
export const ID_RULES: Record<string, Rule> = {
  "PhilSys (National ID)": { hint: "16 digits", ok: (v) => /^\d{12,16}$/.test(digits(v)) },
  UMID: { hint: "12-digit CRN", ok: (v) => /^\d{10,12}$/.test(digits(v)) },
  "SSS/GSIS ID": { hint: "10–11 digits", ok: (v) => /^\d{9,12}$/.test(digits(v)) },
  "PRC ID": { hint: "7 digits", ok: (v) => /^\d{6,9}$/.test(digits(v)) },
  "Driver's License": {
    hint: "e.g. N01-23-456789",
    ok: (v) => /^[A-Za-z0-9]{9,14}$/.test(alnum(v)),
  },
  Passport: {
    hint: "e.g. P1234567A",
    ok: (v) => /^[A-Za-z0-9]{7,10}$/.test(compact(v)),
  },
  "Voter's ID": { hint: "numeric (10–22 digits)", ok: (v) => /^\d{10,22}$/.test(digits(v)) },
  "Postal ID": { hint: "alphanumeric", ok: (v) => /^[A-Za-z0-9]{7,16}$/.test(alnum(v)) },
};
